...
BRANCH_GITHUB = "main"
VERSAO = "v5.0.0 (Painel Completo - Real)"
DATA_VERSAO = "15/06/2026"
...
def buscar_dados_reais():
    # Inicialização das estruturas para todas as fases da Copa do Mundo
    estrutura_copa = {
        "grupos": {1: [], 2: [], 3: []},
        "r32": [], "r16": [], "r8": [], "r4": [], "third": [], "final": []
    }
    try:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] [LIVE] Sincronizando tabela completa da FIFA...")
        ...
        dados = response.json()
        matches = dados if isinstance(dados, list) else dados.get("matches", [])
        
        for match in matches:
            t1_en = match.get("HomeTeam", "")
            t2_en = match.get("AwayTeam", "")
            ...
            match_num = match.get("MatchNumber", 0)
            ...
            jogo_dict = {
                ...
                "est": match.get("Location", "Estádio FIFA"),
                "g1": g1 if g1 is not None else "", "g2": g2 if g2 is not None else "", 
                "encerrado": encerrado
            }

            # Distribuição Inteligente por Fases (Baseado no Regulamento FIFA 104 Jogos)
            if match_num <= 72: # Fase de Grupos
                if match_num <= 24: rodada = 1
                elif match_num <= 48: rodada = 2
                else: rodada = 3
                estrutura_copa["grupos"][rodada].append(jogo_dict)
            elif match_num <= 88:
                estrutura_copa["r32"].append(jogo_dict) # Dezesseis-avos
            elif match_num <= 96:
                estrutura_copa["r16"].append(jogo_dict) # Oitavas
            elif match_num <= 100:
                estrutura_copa["r8"].append(jogo_dict)  # Quartas
            elif match_num <= 102:
                estrutura_copa["r4"].append(jogo_dict)  # Semifinais
            elif match_num == 103:
                estrutura_copa["third"].append(jogo_dict) # 3º Lugar
            elif match_num == 104:
                estrutura_copa["final"].append(jogo_dict) # Grande Final

        return estrutura_copa
    except Exception as e:
        print(f"❌ Erro ao processar árvore da Copa: {e}")
        return None
...
def renderizar_tabela_jogos(lista_jogos):
    html_jogos = ""
    if not lista_jogos:
        return "<tr><td colspan='6' class='center' style='color:#64748b; padding:20px;'>Nenhum jogo agendado ou dados indisponíveis.</td></tr>"
...
<head>
    ...
    <title>Central Completa da Copa do Mundo 2026 - RPC</title>
    ...
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, sans-serif; }}
        ...
        footer {{ position: fixed; bottom: 0; left: 0; width: 100%; background-color: #1e3a8a; color: #ffffff; padding: 16px; text-align: center; font-size: 13px; font-weight: 500; box-shadow: 0 -4px 10px rgba(0,0,0,0.15); z-index: 1000; letter-spacing: 0.3px; }}
...
<body>
    ...
    <div class="container">
        ...
        <div class="status-bar">
            <div><strong>Sincronização FIFA Real-Time:</strong> {datetime.now().strftime('%d/%m/%Y às %H:%M:%S')}</div>
            <div style="font-weight: bold; color: #0d9488;">● Transmissão RPC Online</div>
        </div>
    ...
    html += """
        ...
        <div id="eliminatorias" class="tab-content">
            <div class="subtabs-container">
                <button class="subtab-btn active" onclick="switchSubTab('fase-32', event)">Dezesseis-avos (1/16)</button>
    ...
    <footer>
        <div>Oferecimento: <strong>RPC - Reinaldo Pinheiro Consultoria</strong> - Ajude a ter novos projetos doando no pix: <span class="pix-destaque">doe@reinaldopinheiro.com.br</span></div>
    </footer>
...
def fazer_upload_github():
    try:
        ...
        if repo.is_dirty(untracked_files=False):
            repo.index.commit(f"Tabela Completa Real - {horario}")
            origem = repo.remote(name='origin')
            origem.push(BRANCH_GITHUB, kill_after_timeout=15)
            print(f"[{horario}] ↑ Painel 100% completo publicado em reinaldpinheiro.com.br!")
        else:
            print(f"[{horario}] ✅ Sem novas alterações nos placares oficiais.")
    except Exception as e:
        print(f"❌ Erro Git Deploy: {e}")

if __name__ == "__main__":
    print("=" * 60)
    print("ROBÓ CORE ATIVO - TODOS OS 104 JOGOS REAIS DA COPA 2026")
    print("=" * 60)
    ...
    while True:
        ...
        if estrutura_dados:
            ...
            print("⚠️ Aguardando resposta do feed esportivo para a próxima varredura...")
